...
export const WorkOrderForm: React.FC<Props> = ({ data, requiredFields = {}, onChange, onOpenSettings }) => {
  ...
  // --- Estados para el catálogo de vehículos (catalog_vehicle, local) ---
  ...
  // Años disponibles (generados localmente, no requieren lecturas).
  ...
  // 12. Cargar MARCAS del catálogo local (catalog_vehicle) según el AÑO.
  useEffect(() => {
    if (data.year && data.year.trim()) {
      vehicleCatalogService.getMakes(data.year)
        .then(setVehicleMakes)
  ...
  // 13. Cargar MODELOS del catálogo local según MARCA + AÑO.
  useEffect(() => {
    if (data.mark && data.mark.trim() && data.year && data.year.trim()) {
      setIsLoadingModels(true);
      vehicleCatalogService.getModels(data.year, data.mark)
  ...
  // 14. Cargar BODY del catálogo local según AÑO + MARCA + MODELO.
  useEffect(() => {
    if (
      data.year && data.mark && data.model &&
      data.year.trim() && data.mark.trim() && data.model.trim()
    ) {
      vehicleCatalogService.getBodies(data.year, data.mark, data.model)
        .then(setVehicleBodyClasses)
        .catch(() => setVehicleBodyClasses([]));
    } else {
      setVehicleBodyClasses([]);
    }
  }, [data.year, data.mark, data.model]);
